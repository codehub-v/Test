...
const ProductionForm = () => {
  ...
  const fetchMeta = async () => {
    try {
      const [bomResponse, customerResponse] = await Promise.all([
        api.get("/master/meta/bom/"),
        api.get("/master/meta/customers/"),
      ]);

      setBoms(bomResponse.data.results || bomResponse.data || []);

      setCustomers(
        customerResponse.data.results || customerResponse.data || [],
      );
    } catch (error) {
      console.error("Failed to load production form data:", error);

      setError("Unable to load BOM or customer information.");
    }
  };
  ...
  const fetchProductionOrder = async () => {
    try {
      ...
      setFormData({
        ...
        product: data.product_details.id || "",
        ...
        customer: data.customer_details?.id || data.customer || "",
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.production_no.trim()) {
      setError("Production number is required.");
      return;
    }
    ...
    if (!formData.product) {
      setError("Product / BOM is required.");
      return;
    }
    ...
    try {
      ...
      if (isEdit) {
        await api.put(`/inventory/order/${id}/`, payload);
      } else {
        await api.post("/inventory/order/", payload);
      }
      ...
      if (responseData && typeof responseData === "object") {
        const message = Object.values(responseData).flat().join(" ");

        setError(message || "Failed to save production order.");
      } else {
        ...
        <div className="production-title-section">
          ...
          <div>
            <h2 className="production-title">
              {isEdit ? "Edit Production Order" : "Add Production Order"}
            </h2>
      ...
      <form onSubmit={handleSubmit}>
        <div className="production-card">
          ...
          <div className="production-form-content">
            ...
            <div className="production-row-two">
              <div className="production-form-section">
                <label className="production-label">Status</label>
                ...
                <small className="production-help-text">
                  Status is updated automatically through the production
                  workflow.
                </small>
              </div>

              <div className="production-form-section">
                <label className="production-label">Remarks</label>
        ...
        <div className="production-footer">
          ...
          <button
            ...
            className={`production-save-button ${
              saving ? "production-disabled-button" : ""
            }`}
          >
            <Save size={16} />
