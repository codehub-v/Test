...
const ProductionList = () => {
  ...
  const fetchProductions = async () => {
    try {
      ...
      const response = await api.get("inventory/order/", { params });
  ...
  useEffect(() => {
    fetchProductions();
  }, [currentPage, search, status, productionLine]);
  ...
  const handleStatusClick = (production) => {
    const nextStatus = getNextStatus(production.status);
  ...
  const confirmStatusUpdate = async () => {
    ...
    try {
      ...
      await api.patch(`inventory/order/${selectedProduction.id}/`, {
        status: selectedProduction.nextStatus,
      });
      ...
      let message = "Unable to update production status.";

      if (Array.isArray(responseData)) {
        message = responseData.join(" ");
      } else if (Array.isArray(responseData?.detail)) {
        message = responseData.detail.join(" ");
    ...
    <div style={styles.page}>
      ...
      <div style={styles.header}>
        <div style={styles.titleSection}>
          <h2 style={styles.title}>Production</h2>
      ...
      <div style={styles.filterCard}>
        ...
        <div style={styles.filterGroup}>
          <label style={styles.filterLabel}>Production Line</label>
            ...
            <tbody>
              {loading ? (
                ...
                data.map((production, index) => {
                  const statusStyle = getStatusStyle(production.status);

                  const canProgress = Boolean(getNextStatus(production.status));
                          ...
                          {canProgress && (
                            <button
                              ...
                              title={`Move to ${getStatusLabel(
                                getNextStatus(production.status),
                              )}`}
                            >
                              <Play size={15} />
                            </button>
      ...
      {showStatusModal && selectedProduction && (
        <div style={styles.modalOverlay} onClick={closeStatusModal}>
          <div style={styles.modal} onClick={(e) => e.stopPropagation()}>
            <div
              style={
                selectedProduction.action === "cancel"
            ...
            <p style={styles.modalText}>
              ...
              <strong>{selectedProduction.production_no}</strong>?
            </p>
            ...
            <div style={styles.statusChange}>
              ...
              >
                {getStatusLabel(selectedProduction.nextStatus)}
              </span>
            </div>
            ...
            {statusError && (
              <div style={styles.errorBox}>
                <div style={styles.errorHeader}>
                  ...
                  <span>Insufficient Material</span>
                </div>

                <p style={styles.errorMessage}>{statusError}</p>
                ...
                {showMaterialError && (
                  <button
                    ...
                    onClick={() => {
                      ...
                      navigate(`/orders/detials/${selectedProduction.id}`);
                    }}
                    style={styles.materialDetailsButton}
                  >
